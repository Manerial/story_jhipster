...
/**
 * Service Implementation for managing {@link com.jher.nid_aux_histoires.domain.BookStatus}.
 */
@Service
@Transactional
public class BookStatusServiceImpl implements BookStatusService {
    ...
    @Override
    public BookStatusDTO update(BookStatusDTO bookStatusDTO) {
        log.debug("Request to update BookStatus : {}", bookStatusDTO);
        BookStatus bookStatus = bookStatusMapper.toEntity(bookStatusDTO);
        bookStatus = bookStatusRepository.save(bookStatus);
        return bookStatusMapper.toDto(bookStatus);
    }
    ...
    @Override
    public Optional<BookStatusDTO> partialUpdate(BookStatusDTO bookStatusDTO) {
        log.debug("Request to partially update BookStatus : {}", bookStatusDTO);

        return bookStatusRepository
            .findById(bookStatusDTO.getId())
            .map(existingBookStatus -> {
                bookStatusMapper.partialUpdate(existingBookStatus, bookStatusDTO);

                return existingBookStatus;
            })
            .map(bookStatusRepository::save)
    ...
    public Page<BookStatusDTO> findAll(Pageable pageable) {
        log.debug("Request to get all BookStatuses");
