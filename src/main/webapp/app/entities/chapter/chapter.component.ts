...
export class ChapterComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.chapterService
      ...
      .subscribe(
        (res: HttpResponse<IChapter[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInChapters(): void {
    this.eventSubscriber = this.eventManager.subscribe('chapterListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IChapter[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/entities/chapter'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
