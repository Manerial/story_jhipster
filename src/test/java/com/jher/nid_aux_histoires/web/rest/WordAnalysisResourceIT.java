...
public class WordAnalysisResourceIT {
	...
	public void createWordAnalysis() throws Exception {
		...
		restWordAnalysisMockMvc.perform(post("/api/word-analyses").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(wordAnalysisDTO))).andExpect(status().isCreated());
	...
	public void createWordAnalysisWithExistingId() throws Exception {
		...
		restWordAnalysisMockMvc
				.perform(post("/api/word-analyses").contentType(MediaType.APPLICATION_JSON)
	...
	public void getAllWordAnalyses() throws Exception {
		...
		restWordAnalysisMockMvc.perform(get("/api/word-analyses?sort=id,desc")).andExpect(status().isOk())
	...
	public void getWordAnalysis() throws Exception {
		...
		restWordAnalysisMockMvc.perform(get("/api/word-analyses/{id}", wordAnalysis.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(jsonPath("$.id").value(wordAnalysis.getId().intValue()))
				.andExpect(jsonPath("$.type").value(DEFAULT_TYPE)).andExpect(jsonPath("$.name").value(DEFAULT_NAME))
	...
	public void updateWordAnalysis() throws Exception {
		...
		// Disconnect from session so that the updates on updatedWordAnalysis are not
		...
		restWordAnalysisMockMvc.perform(put("/api/word-analyses").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(wordAnalysisDTO))).andExpect(status().isOk());
	...
	public void updateNonExistingWordAnalysis() throws Exception {
		...
		restWordAnalysisMockMvc
				.perform(put("/api/word-analyses").contentType(MediaType.APPLICATION_JSON)
						.content(TestUtil.convertObjectToJsonBytes(wordAnalysisDTO)))
				.andExpect(status().isBadRequest());
	...
	public void deleteWordAnalysis() throws Exception {
		...
		restWordAnalysisMockMvc
				.perform(delete("/api/word-analyses/{id}", wordAnalysis.getId()).accept(MediaType.APPLICATION_JSON))
