...
import com.jher.nid_aux_histoires.service.mapper.UserMapper;
import com.jher.nid_aux_histoires.web.rest.vm.ManagedUserVM;
...
public class UserResourceIT {
	...
	public static User createEntity(EntityManager em) {
		...
		authorityA.setName(AuthoritiesConstants.AUTHOR);
		auth.add(authorityA);
	...
	@Test
	@Transactional
	public void getUserLight() throws Exception {
		// Initialize the database
		userRepository.saveAndFlush(user);
		...
		// Get the user
		restUserMockMvc.perform(get("/api/user_light/login/{login}", user.getLogin())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(jsonPath("$.login").value(user.getLogin()))
				.andExpect(jsonPath("$.firstName").value(DEFAULT_NULL_VALUE))
				.andExpect(jsonPath("$.lastName").value(DEFAULT_NULL_VALUE))
				.andExpect(jsonPath("$.email").value(DEFAULT_NULL_VALUE))
				.andExpect(jsonPath("$.imageUrl").value(DEFAULT_IMAGEURL))
				.andExpect(jsonPath("$.langKey").value(DEFAULT_NULL_VALUE));
	}

	@Test
	@Transactional
	public void getAuthorsLight() throws Exception {
		// Initialize the database
		userRepository.saveAndFlush(user);
		...
		// Get the user
		restUserMockMvc.perform(get("/api/user_light/authors")).andExpect(status().isOk()).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(jsonPath("$.[*].login").value(hasItem(DEFAULT_LOGIN)))
				.andExpect(jsonPath("$.[*].firstName").value(hasItem(DEFAULT_NULL_VALUE)))
				.andExpect(jsonPath("$.[*].lastName").value(hasItem(DEFAULT_NULL_VALUE)))
				.andExpect(jsonPath("$.[*].email").value(hasItem(DEFAULT_NULL_VALUE)))
				.andExpect(jsonPath("$.[*].imageUrl").value(hasItem(DEFAULT_IMAGEURL)))
				.andExpect(jsonPath("$.[*].langKey").value(hasItem(DEFAULT_NULL_VALUE)));
	}
