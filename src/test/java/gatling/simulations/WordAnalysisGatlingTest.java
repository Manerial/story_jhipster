...
/**
 * Performance test for the WordAnalysis entity.
 */
public class WordAnalysisGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all wordAnalyses").get("/api/word-analyses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new wordAnalysis")
                        .post("/api/word-analyses")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"type\": \"SAMPLE_TEXT\"" + ", \"name\": \"SAMPLE_TEXT\"" + ", \"analysis\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_wordAnalysis_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created wordAnalysis").get("${new_wordAnalysis_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created wordAnalysis").delete("${new_wordAnalysis_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WordAnalysis entity").exec(scn);
