 ...
 */
public class ChapterGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all chapters").get("/api/chapters").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new chapter")
                        .post("/api/chapters")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"description\": \"SAMPLE_TEXT\"" + ", \"number\": 0" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_chapter_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created chapter").get("${new_chapter_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created chapter").delete("${new_chapter_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Chapter entity").exec(scn);
