...
/**
 * Performance test for the BookStatus entity.
 */
public class BookStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all bookStatuses").get("/api/book-statuses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bookStatus")
                        .post("/api/book-statuses")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"finished\": null" + ", \"favorit\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_bookStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created bookStatus").get("${new_bookStatus_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created bookStatus").delete("${new_bookStatus_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BookStatus entity").exec(scn);
